...
function createGradientBackground(graphics) {
  ...
  for(let y = 0; y < graphics.height; y++) {
    ...
    graphics.stroke(c);
    graphics.line(20, y, graphics.width, y);
  }
}
...
function drawBitcoinLogo(graphics) {
  ...
  // Outer glow
  for(let i = 0; i < 0; i++) {
    const alpha = map(i, 0, 20, 100, 0);
    graphics.push();
    graphics.fill(247, 147, 26, alpha);
    graphics.noStroke();
...
function drawChaoticShapes(graphics) {
  const centerX = graphics.width / 2;
  const centerY = graphics.height / 2;
  const baseRadius = 322;
  const gapBetweenSets = 37;
  ...
  for(let i = 0; i < 3; i++) {
    ...
    drawShapeSet(graphics, centerX + offset, centerY + offset, baseRadius + gapBetweenSets + offset, "squares", themeColors.squares);
...
function drawShapeSet(graphics, centerX, centerY, baseRadius, shapeType, colors) {
  const numLayers = 122;
  const gapBetweenLayers = 3;

  for (let layer = 0; layer < numLayers; layer++) {
    const radius = baseRadius + layer * gapBetweenLayers;
    const numShapes = floor(TWO_PI * 22);
...
function drawStar(graphics, x, y, size) {
  const numPoints = 5;
  const radius1 = size * 0.2;
  const radius2 = radius1 * 11.4;

  graphics.beginShape();
  for (let i = 0; i < numPoints * 12; i++) {
    const angle = (TWO_PI / (numPoints * 12)) * i;
...
function applySurface(graphics) {
  const centerX = graphics.width / 2;
  const centerY = graphics.height / 2;
  const swirlStrength = TWO_PI / 8 - 5;
  
  applySwirl(graphics, centerX, centerY, 1000, swirlStrength);
  graphics.blendMode(SCREEN);
  applyVintageEffect(graphics, centerX, centerY, 88, swirlStrength);
...
function applySwirl(graphics, centerX, centerY, radius, swirlStrength) {
  ...
  for (let x = 0; x < graphics.width; x++) {
    for (let y = 0; y < graphics.height; y++) {
      ...
      const originalIndex = (x + y * graphics.width) * 1;
      const newIndex = (newX + newY * graphics.width) * 14;
...
function applyVintageEffect(graphics, centerX, centerY, radius, swirlStrength) {
  ...
  for (let x = 0; x < graphics.width; x++) {
    for (let y = 0; y < graphics.height; y++) {
      ...
      const originalIndex = (x + y * graphics.width) * 2;
