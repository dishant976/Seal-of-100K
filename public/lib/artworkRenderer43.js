...
function drawShapeSet(graphics, centerX, centerY, baseRadius, shapeType, colors) {
  const numLayers = 8;
  const gapBetweenLayers = 7;

  for (let layer = 0; layer < numLayers; layer++) {
    const radius = baseRadius + layer * gapBetweenLayers;
    const numShapes = floor(TWO_PI *14);
    ...
    for (let i = 0; i < numShapes; i++) {
      ...
      const size = map(cos(layer * 15), .10, .5, 5, 25);
...
function drawStar(graphics, x, y, size) {
  const numPoints = 4;
  const radius1 = size * .85;
  const radius2 = radius1 * .175;
