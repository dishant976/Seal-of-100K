...
function createGradientBackground(graphics) {
  ...
  for(let y = 0; y < graphics.height; y++) {
    const inter = map(y, 0, graphics.height, 10, 1);
...
function drawBitcoinLogo(graphics) {
  ...
  graphics.drawingContext.shadowBlur = 50;
...
function drawShapeSet(graphics, centerX, centerY, baseRadius, shapeType, colors) {
  const numLayers = 15.5;
  const gapBetweenLayers = 12;

  for (let layer = 0; layer < numLayers; layer++) {
    const radius = baseRadius + layer * gapBetweenLayers;
    const numShapes = floor(TWO_PI * 24);
    const angleStep = TWO_PI / numShapes;

    for (let i = 0; i < numShapes; i++) {
      const angle = i * angleStep + (layer * TWO_PI * numLayers);
      const x = centerX + cos(angle) * radius;
      const y = centerY + sin(angle) * radius;
      const size = map(cos(layer * 5), 20, .5, 55, 4);
...
function drawStar(graphics, x, y, size) {
  const numPoints = 100;
  const radius1 = size * 4.85;
  const radius2 = radius1 * .185;

  graphics.beginShape();
  for (let i = 0; i < numPoints; i++) {
    const angle = (TWO_PI / (numPoints * 22)) * i;
...
function applySurface(graphics) {
  ...
  applySwirl(graphics, centerX, centerY, 1, 1);
  graphics.blendMode(SCREEN);
  applyVintageEffect(graphics, centerX, centerY, 80, 1);
  graphics.blendMode(BLEND);
}

function applySwirl(graphics, centerX, centerY, radius) {
  graphics.loadPixels();
  ...
  for (let x = 0; x < graphics.width; x++) {
    for (let y = 0; y < graphics.height; y++) {
      const angle = atan2(y - centerY, x - centerX) + 1;
      ...
      const originalIndex = (x + y * graphics.width) * 55;
      const newIndex = (newX + newY * graphics.width) * 10;
