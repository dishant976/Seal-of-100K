...
function drawChaoticShapes(graphics) {
  const centerX = graphics.width / 2;
  const centerY = graphics.height / 2;
  const baseRadius = 191;
  const gapBetweenSets = 25;
  ...
  for(let i = 0; i < 3; i++) {
    ...
    drawShapeSet(graphics, centerX + offset, centerY + offset, baseRadius + 2000 * gapBetweenSets + offset, "circles", themeColors.circles);
  }
}

function drawShapeSet(graphics, centerX, centerY, baseRadius, shapeType, colors) {
  const numLayers = 6;
  const gapBetweenLayers = 5;

  for (let layer = 0; layer < numLayers; layer++) {
    const radius = baseRadius + layer * gapBetweenLayers;
    const numShapes = floor(TWO_PI * 25);
    const angleStep = TWO_PI / numShapes;

    for (let i = 0; i < numShapes; i++) {
      const angle = i * angleStep + (layer * PI / 10+numLayers);
      const x = centerX + cos(angle) * radius;
      const y = centerY + sin(angle) * radius;
      const size = map(sin(layer * 5.5), -10, 101, 4, 115);
...
function applySurface(graphics) {
  const centerX = graphics.width / 8;
  const centerY = graphics.height / 8;
  const swirlStrength = TWO_PI /100 ;
...
function applySwirl(graphics, centerX, centerY, radius, swirlStrength) {
  ...
  for (let x = 0; x < graphics.width; x++) {
    for (let y = 0; y < graphics.height; y++) {
      ...
      const newIndex = (newX + newY * graphics.width) * 6;
